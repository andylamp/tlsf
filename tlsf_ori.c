...
static __inline__ bhdr_t *FIND_SUITABLE_BLOCK(tlsf_ori_t * _tlsf, int *_fl, int *_sl);
...
/******************************************************************/
size_t init_tlsf_ori_pool(size_t mem_pool_size, void *mem_pool)
{
/******************************************************************/
    tlsf_ori_t *tlsf;
    bhdr_t *b, *ib;

    if (!mem_pool || !mem_pool_size || mem_pool_size < sizeof(tlsf_ori_t) + BHDR_OVERHEAD * 8) {
        ERROR_MSG("init_tlsf_ori_pool (): memory_pool invalid\n");
        return -1;
    }

    if (((unsigned long) mem_pool & PTR_MASK)) {
        ERROR_MSG("init_tlsf_ori_pool (): mem_pool must be aligned to a word\n");
        return -1;
    }
    tlsf = (tlsf_ori_t *) mem_pool;
    /* Check if already initialised */
    if (tlsf->tlsf_signature == TLSF_SIGNATURE) {
        mp = mem_pool;
        b = GET_NEXT_BLOCK(mp, ROUNDUP_SIZE(sizeof(tlsf_ori_t)));
    ...
    memset(mem_pool, 0, sizeof(tlsf_ori_t));
    ...
    ib = process_area(GET_NEXT_BLOCK
                      (mem_pool, ROUNDUP_SIZE(sizeof(tlsf_ori_t))), ROUNDDOWN_SIZE(mem_pool_size - sizeof(tlsf_ori_t)));
...
{
/******************************************************************/
    tlsf_ori_t *tlsf = (tlsf_ori_t *) mem_pool;
...
/******************************************************************/
#if TLSF_STATISTIC
    return ((tlsf_ori_t *) mem_pool)->used_size;
#else
    return 0;
#endif
}
...
/******************************************************************/
#if TLSF_STATISTIC
    return ((tlsf_ori_t *) mem_pool)->max_size;
#else
    return 0;
#endif
}
...
/******************************************************************/
void del_tlsf_ori_pool(void *mem_pool)
{
/******************************************************************/
    tlsf_ori_t *tlsf = (tlsf_ori_t *) mem_pool;
...
/******************************************************************/
void *tlsf_ori_malloc(size_t size)
{
/******************************************************************/
    void *ret;
    ...
    if (!mp) {
        ...
        area_size = sizeof(tlsf_ori_t) + BHDR_OVERHEAD * 8; /* Just a safety constant */
        area_size = (area_size > DEFAULT_AREA_SIZE) ? area_size : DEFAULT_AREA_SIZE;
        area = get_new_area(&area_size);
        if (area == ((void *) ~0))
            return NULL;        /* Not enough system memory */
        init_tlsf_ori_pool(area_size, area);
    }
#endif

    TLSF_ACQUIRE_LOCK(&((tlsf_ori_t *)mp)->lock);

    ret = malloc_ex(size, mp);

    TLSF_RELEASE_LOCK(&((tlsf_ori_t *)mp)->lock);

    return ret;
}

/******************************************************************/
void tlsf_ori_free(void *ptr)
{
/******************************************************************/
...
/******************************************************************/
void *tlsf_ori_realloc(void *ptr, size_t size)
{
/******************************************************************/
    void *ret;
...
/******************************************************************/
void *tlsf_ori_calloc(size_t nelem, size_t elem_size)
{
/******************************************************************/
    void *ret;
...
extern void print_tlsf(tlsf_ori_t * tlsf);
void print_all_blocks(tlsf_ori_t * tlsf);
...
void print_tlsf(tlsf_ori_t * tlsf)
{
    bhdr_t *next;
    int i, j;
